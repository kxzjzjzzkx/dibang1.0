...
#include <iostream>
// 幻数定义
const int MD5::kA = 0x67452301;
const int MD5::kB = 0xefcdab89;
const int MD5::kC = 0x98badcfe;
const int MD5::kD = 0x10325476;

const unsigned long long MD5::k_ti_num_integer = 4294967296;

// function: CycleMoveLeft
// @param src_num:要左移的数
// @param bit_num_to_move:要移动的bit位数
// @return  循环左移后的结果数
UInt32 MD5::CycleMoveLeft(UInt32 src_num, int bit_num_to_move) {
...
UInt32 MD5::FillData(const char *in_data_ptr, int data_byte_len, char** out_data_ptr) {
...
void MD5::RoundF(char *data_BIT_OF_GROUP_ptr, ParamDynamic & param) {
...
void MD5::RoundG(char *data_BIT_OF_GROUP_ptr, ParamDynamic & param) {
	UInt32 *M = reinterpret_cast<UInt32*>(data_BIT_OF_GROUP_ptr);
	int s[] = { 5, 9, 14, 20 };
	for (int i = 0; i < 16; ++i) {
...
void MD5::RoundH(char *data_BIT_OF_GROUP_ptr, ParamDynamic & param) {
	UInt32 *M = reinterpret_cast<UInt32*>(data_BIT_OF_GROUP_ptr);
	int s[] = { 4, 11, 16, 23 };
	for (int i = 0; i < 16; ++i) {
...
void MD5::RoundI(char *data_BIT_OF_GROUP_ptr, ParamDynamic & param) {
	UInt32 *M = reinterpret_cast<UInt32*>(data_BIT_OF_GROUP_ptr);
	int s[] = { 6, 10, 15, 21 };
	for (int i = 0; i < 16; ++i) {
...
void MD5::RotationCalculate(char *data_512_ptr, ParamDynamic & param) {
